...
def check_file(path: str, content: str) -> FileReview:
    ...
    if _FETCH.search(code):
        fr.violations.append("direct fetch() call (route via ApiService — context.md §4)")
    # A component must not talk HTTP directly; it goes through ApiService.
    if _COMPONENT.search(code) and _HTTPCLIENT.search(code):
    ...
    # reuse: is a canonical primitive / reusable building block actually imported?
    ...
    if not reuses:
        fr.violations.append("does not reuse any canonical primitive/service (context.md §3)")
    ...
    if not _RETURN_TYPE.search(code):
        fr.warnings.append("no explicit return type on the exported function")
    return fr
...
def write_review_md(exec_result, review: ReviewResult, out_dir: Path) -> Path:
    out_dir.mkdir(parents=True, exist_ok=True)
    md_path = out_dir / "REVIEW.md"
    status = "PASS ✅ — ready for human review" if review.passed else "FAIL ❌ — fix before review"
    lines = [
        "# REVIEW.md — Phase 4 (Test & Human-in-the-Loop)",
        "",
        f"**Automated compliance suite: {status}**",
        "",
